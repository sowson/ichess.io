...
#define CH_ACTION_SPACE 4672

typedef struct {
    int    move_count;
    char   moves[256][8];   // legal move strings
    float  visits[256];     // visit counts N(s,a)
    float  q_root;          // average value at root
    int    best_index;      // argmax(N)
} ch_mcts_az;

static ch_mcts_az g_last_res = {0};
